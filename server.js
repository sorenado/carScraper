...
const sendConfirmation = require('./sendConfirmationEmail'); // Sending Confirmation email returns the confirmation code sent
const app = express();
const port = 8080;
...
app.post('/join-button-form', async (req, res) => {
    try {
        ...
        const generatedCode = sendConfirmation(email);
        ...
        pendingUser.set(email, userData);
    }
    catch (err) {
        console.error(err);
    }
